...
int check1(vector<int> &arr , int target , int mid) {
    return arr[mid] > target;
}
...
int upper_bound(vector<int>& arr, int target) {    
    ...
    while (lo <= hi) {
        int mid = lo + (hi - lo) / 2;
        if (check1(arr , target , mid)) ans = mid , hi = mid - 1;
        else lo = mid + 1;
    }
    return ans;
}
vector<int> searchRange(vector<int>& nums, int target) {
    ...
    int ans2 = upper_bound(nums , target);

    vector<int> ans; if (ans1 == nums.size()) return {-1 , -1};
    if (nums[ans1] != target) return {-1 , -1};
    return {ans1 , ans2 - 1};
}
